    ...
    try:
        ...
        for class_name in class_names:
            try:
                ...
                result = client.query.get(class_name, ["id"]).with_limit(3).do()
                
                if "data" in result and "Get" in result["data"] and class_name in result["data"]["Get"] and result["data"]["Get"][class_name]:
                    samples = result["data"]["Get"][class_name]
                    print(f"{class_name}: {len(samples)} sample(s) retrieved")
                    
                    # For each sample, get the full object
                    for i, sample in enumerate(samples):
                        if "_additional" in sample and "id" in sample["_additional"]:
                            obj_id = sample["_additional"]["id"]
                            obj = client.data_object.get_by_id(class_name, obj_id)
                            
                            # Print a summarized version of the object
                            props = obj.get("properties", {})
                            summary = {k: v[:50] + "..." if isinstance(v, str) and len(v) > 50 else v 
                                     for k, v in props.items()}
                            print(f"  Sample {i+1}: {json.dumps(summary, indent=2)}")
                    
                else:
                    print(f"{class_name}: No data returned")
            except Exception as e:
                print(f"{class_name}: No objects found")
