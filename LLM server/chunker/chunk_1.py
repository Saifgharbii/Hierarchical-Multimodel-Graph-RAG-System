...
def process_file_task(file_path, output_folder, file_index, window_size, step_size, prefix):
    ...
    file_output_folder = os.path.join(output_folder, f"file{file_index}")
    ...
    if success:
        print(f"Processing completed successfully for file {file_index} . Results in: {file_output_folder}")
    else:
        print(f"Document processing failed for file {file_index}.")
    
    return file_index, success, file_output_folder
...
def main():
    ...
    if os.path.isfile(input_path):
        if not input_path.lower().endswith('.json'):
            ...
            return      
        file_output_folder = os.path.join(output_folder, "file0")
        ...
        if success:
            print(f"Processing completed successfully. Results in: {file_output_folder}")
        else:
